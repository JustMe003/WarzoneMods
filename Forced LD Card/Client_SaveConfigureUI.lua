...
function Client_SaveConfigureUI(alert, addCard)
    ...
    if Mod.Settings.NumPieces < 1 then alert("LD Card: The number of pieces the card is divided into must be bigger or equal to 1"); end
    Mod.Settings.PiecesPerTurn = piecesPerTurn.GetValue();
    if Mod.Settings.PiecesPerTurn < 0 then alert("LD Card: The number of pieces awarded per turn must be bigger or equal to 1"); end
    Mod.Settings.StartingPieces = startingPieces.GetValue();
    if Mod.Settings.StartingPieces < 0 then alert("LD Card: The number of pieces given to each player at the start of the game must be a positive number"); end
    Mod.Settings.Weight = weight.GetValue();
    if Mod.Settings.Weight < 0 then alert("LD Card: The weight of the card must be a positive number"); end
    Mod.Settings.Duration = duration.GetValue();
    if Mod.Settings.Duration < 1 then alert("LD Card: The card duration must be bigger or equal to 1"); end

...
function Client_GameRefresh(game)
	...
	if Mod.PublicGameData.VersionNumber == nil or Mod.PublicGameData.VersionNumber < 6 then game.SendGameCustomMessage("Refreshing page...", {Type="updateData"}, function(payload) UI.Alert(payload.Type .. "\n" .. payload.Message); end); return; end
...
function showAlert(game)
	...
	if playerData.Notifications.Messages ~= nil and count(playerData.Notifications.Messages, function(t) return #t; end) > 0 then
		s = s .. "You have " .. count(playerData.Notifications.Messages, function(t) return #t; end) .. " unread messages in faction chats";
	end
	if #s > 0 then
		UI.Alert(s);
	end

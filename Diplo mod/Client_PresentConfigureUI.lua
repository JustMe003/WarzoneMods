...
function showFactionConfig(faction)
	...
	newButton(win .. "SetFactionLeader", line, getFactionLeader(faction), function() PromptForFactionLeader(faction) end, "Yellow");
	...
	newButton(win .. "AddSlots", line, "Add slot", function() promptForAddingSlot(faction); end, "Green");
	newButton(win .. "RemoveSlots", line, "Remove slot", function() promptForRemovingSlot(faction); end, "Red");
...
function PromptForFactionLeader(faction)
	local payload = {};
	for i = 0, 49 do
		if config.SlotInFaction[i] == nil then
			table.insert(payload, {text=getSlotName(i), selected=function() config.Factions[faction].FactionLeader = i; table.insert(config.Factions[faction].FactionMembers, i); showFactionConfig(faction); end});
		end
	end

...
function showFactionJoinRules()
    ...
    window(win);
    local vert = newVerticalGroup("Vert", "root");
    newLabel(win .. "text", vert, "Depending on the game, when clicking the join button for a Faction you actually join the Faction or send a join request to its Faction leader. But you cannot join every Faction, here is the following points where the mod looks at before letting you join.\n\nYou cannot join a Faction if you're at war with one of its members. If you do want to join the Faction, you must first make sure you're at least in peace with all its members.\nIf the setting 'Fair Factions' is on, then you're not allowed to join a Faction when your income + the income of all members combined is bigger than a certain percentage of the total income of all players. This is be used to avoid the best players in the game to team up against all the others", "#CCCCCC");
    newButton(win .. "Return", vert, "Return", forcedRules, "Orange");
end
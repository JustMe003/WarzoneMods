...
function Client_PresentMenuUI(rootParent, setMaxSize, setScrollable, Game, close, calledFromGameRefresh)
	...
	if Mod.PublicGameData.VersionNumber == nil or Mod.PublicGameData.VersionNumber < 6 then UI.Alert("This mod has to update first before you can use it again!"); close(); return; end
...
function viewFactionRelations(factionName)
	...
	window(win);
	local vert = newVerticalGroup("vert", "root");
	newButton(win .. "label", vert, factionName, function() showFactionDetails(factionName); end, game.Game.Players[Mod.PublicGameData.Factions[factionName].FactionLeader].Color.HtmlColor);

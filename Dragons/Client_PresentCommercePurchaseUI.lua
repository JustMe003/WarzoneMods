...
function Client_PresentCommercePurchaseUI(rootParent, game, close)
    ...
    if b then
        ...
        for _, dragon in pairs(Mod.Settings.Dragons) do
            ...
            UI.CreateEmpty(line).SetFlexibleWidth(1);
            UI.CreateLabel(line).SetText(dragon.Cost .. " gold").SetColor(colors.Orange);
        end
    else
...
function purchaseMain()
    ...
    for _, dragon in pairs(Mod.Settings.Dragons) do
        if not dragon.CanBeBought then
            ...
            CreateLabel(line).SetText("   " .. dragon.Cost .. " gold").SetColor(colors.Orange);
...
function purchaseDragon(dragon)
    ...
    table.insert(orders, index, WL.GameOrderCustom.Create(Game.Us.ID, "Purchased " .. returnA(dragon) .. "'" .. dragon.Name .. "' on " .. selectedTerr.Name, "Dragons_" .. dragon.ID .. "_" .. selectedTerr.ID, {[WL.ResourceType.Gold] = dragon.Cost}, WL.TurnPhase.Deploys + 1));
    Game.Orders = orders;
    Close();
end

...
function terrClicked(terrDetails, dragon)
    if UI.IsDestroyed(selected) or UI.IsDestroyed(label) or UI.IsDestroyed(purchase) then return WL.CancelClickIntercept; end
...
function purchaseDragon(dragon)
    ...
    table.insert(orders, index, WL.GameOrderCustom.Create(Game.Us.ID, "Purchase " .. returnA(dragon) .. "'" .. dragon.Name .. "' on " .. selectedTerr.Name, "Dragons_" .. dragon.ID .. "_" .. selectedTerr.ID, {[WL.ResourceType.Gold] = dragon.Cost}, WL.TurnPhase.Deploys));
    Game.Orders = orders;
    Close();
end

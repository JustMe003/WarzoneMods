        ...
        if #orderResult.ActualArmies.SpecialUnits > 0 then
            ...
            if dragonCount > 0 then
                ...
                for connID, _ in pairs(game.Map.Territories[order.To].ConnectedTo) do
                    if game.ServerGame.LatestStanding.Territories[connID].OwnerPlayerID ~= order.PlayerID then
                        local mod = WL.TerritoryModification.Create(connID);
                        mod.AddArmies = -math.min(game.ServerGame.LatestStanding.Territories[connID].NumArmies.NumArmies, dragonCount * 5);

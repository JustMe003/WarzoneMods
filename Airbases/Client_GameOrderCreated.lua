...
function testForNumberOfCities(game, order, terrID, skipOrder)
    ...
    local cities = 0;
    if structures ~= nil and structures[WL.StructureType.City] ~= nil then
        cities = structures[WL.StructureType.City];
    end
...
function getPurchasedCities(orders, terrID)
    for _, order in pairs(orders) do
        ...
        elseif order.OccursInPhase ~= nil and order.OccursInPhase > WL.TurnPhase.Purchase then
            return 0;
        end
    end
    return 0;

function Server_AdvanceTurn_Order(game, order, orderResult, skipThisOrder, addNewOrder)
    if order.proxyType == "GameOrderPlayCardAirlift" then
        if not hasRequiredCities(game.ServerGame.LatestTurnStanding.Territories[order.ToTerritoryID]) or not hasRequiredCities(game.ServerGame.LatestTurnStanding.Territories[order.FromTerritoryID]) then
            ...
            addNewOrder(WL.GameOrderCustom.Create(order.PlayerID, "Your airlift order was skipped because " .. game.Map.Territories[order.FromTerritoryID].Name .. " and/or " .. game.Map.Territories[order.ToTerritoryID].Name .. " have less than " .. tostring(Mod.Settings.RequiredCities) .. " cities", ""));
        end
    end
end
